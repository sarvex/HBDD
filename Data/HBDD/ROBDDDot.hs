...
showDotLabel context (ROBDD    l v r i _) =
  show (identifier l) ++ show i ++ show (identifier r) ++
  "[label = " ++ show v ++ "];" ++ showDotLabel context l ++ showDotLabel context r
showDotLabel context (ROBDDRef l v r _ _) = showDotLabel context $ lookupUnsafe (ROBDDId l v r) context
...
showDotLinks context (ROBDD    l _ r i _) =
  show (identifier l ) ++ show i ++ show (identifier r)
...
showDotLinks context (ROBDDRef l v r _ _) = showDotLinks context $ lookupUnsafe (ROBDDId l v r) context
...
showDotLabel' (ROBDD left _ right i _)=
  show (identifier left) ++ show i ++ show (identifier right)
showDotLabel' (ROBDDRef left _ right i _)=
